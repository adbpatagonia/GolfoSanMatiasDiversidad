...
MultiVariogram <- function(Z, MyVar, x, y, MyDistance) {
  ...
  P <- xyplot(Var.gamma ~ Var.dist | factor(i),
    col = 1, type = "p", pch = 16,
    data = VarAll,
    xlab = "Distance",
    ylab = "Semi-variogram",
...
Mydotplot <- function(DataSelected) {
  P <- dotplot(as.matrix(as.matrix(DataSelected)),
    ...
    scales = list(
      x = list(relation = "free", draw = TRUE),
...
Mybwplot <- function(Z, MyVar, TargetVar) {
  ...
  P <- bwplot(AllY ~ factor(AllX) | ID,
    horizontal = FALSE,
    ylab = "", xlab = "",
    scales = list(
      alternating = T, cex.lab = 1.5,
...
MyxyplotBin <- function(Z, MyV, NameY1) {
  ...
  P <- xyplot(AllY ~ AllX | factor(AllID),
    col = 1,
...
MyxyplotPolygon <- function(Z, MyV, NameY1) {
  ...
  library(mgcv)
  library(lattice)
  Z <- xyplot(AllY ~ AllX | factor(AllID),
    col = 1,
